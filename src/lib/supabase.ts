...
/**
 * Deeply validates if the Supabase client and its Realtime module are fully instantiated.
 * This prevents the "Cannot read properties of undefined (reading 'channel')" error.
 */
export const isSupabaseReady = (client: unknown): boolean => {
  try {
    const c = client as { channel?: unknown; realtime?: { channel?: unknown } };
    return (
      !!c &&
      typeof c.channel === 'function' &&
      !!c.realtime &&
      typeof c.realtime.channel === 'function'
    );
  } catch {
    return false;
  }
